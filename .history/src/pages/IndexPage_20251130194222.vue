    ...
    onMounted(() => {
      ...
      if ('webkitSpeechRecognition' in win || 'SpeechRecognition' in win) {
        ...
        if (speechRecognition) {
          ...
          // Fehler ignorieren wir hier weitgehend, da es nur Visualisierung ist
    ...
    const blobToBase64 = (blob: Blob): Promise<string> => {
      return new Promise((resolve, reject) => {
        ...
        reader.onloadend = () => {
          ...
          // Entferne den Prefix "data:audio/webm;base64,"
...
3. **Action Items**: ...
    ...
    const copyToClipboard = (t: string) => {
      qCopy(t).then(() => $q.notify({ color: 'green', message: 'Kopiert' }));
    };
    const clearAll = () => {
      transcript.value = '';
      interimTranscript.value = '';
