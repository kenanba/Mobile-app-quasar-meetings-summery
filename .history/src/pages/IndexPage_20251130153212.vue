...
// --- Typ-Definitionen für Web Speech API (fehlen oft in Standard TS Libs) ---
...
// Window Erweiterung für den Zugriff auf die Konstruktoren
...
export default defineComponent({
  ...
  setup() {
    ...
    // Speech Recognition Instance - Jetzt mit richtigem Typ
    ...
    onMounted(() => {
      ...
      // Cast window zu unserem erweiterten Interface
        ...
        recognition.onresult = (event: SpeechRecognitionEvent) => {
          ...
          for (let i = event.resultIndex; i < event.results.length; ++i) {
            if (event.results[i]?.isFinal) {
              finalTranscript += event.results[i]![0].transcript ?? 0;
            }
          }
          if (finalTranscript) {
