...
export default defineComponent({
  ...
  setup() {
    ...
    // --- NEU: Expliziter Permission Check ---
    const checkAudioPermission = async () => {
      try {
        const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
        // Wenn es klappt, sofort stoppen (wir wollten nur die Permission)
        stream.getTracks().forEach((track) => track.stop());
        return true;
      } catch (err) {
        console.error('Mikrofon-Check fehlgeschlagen:', err);
        $q.notify({
          color: 'negative',
          icon: 'mic_off',
          message: 'Mikrofon-Zugriff verweigert. Prüfe deine Android-Einstellungen.',
        });
        return false;
      }
    };
...
.recording-pulse {
  animation: pulse-red 2s infincite;
}
