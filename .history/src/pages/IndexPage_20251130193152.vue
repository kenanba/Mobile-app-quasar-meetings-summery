...
export default defineComponent({
  ...
  setup() {
    ...
    // Speicher für den letzten finalen Satz (Anti-Duplikat)
    ...
    onMounted(() => {
      ...
      if ('webkitSpeechRecognition' in win || 'SpeechRecognition' in win) {
        ...
        recognition.onresult = (event: SpeechRecognitionEvent) => {
          ...
          if (finalChunk) {
            // FIX: Strenge Duplikat-Prüfung für Android
            const cleanFinal = finalChunk.trim();
            const currentTranscript = transcript.value.trim();

            let isDuplicate = false;

            // 1. Exaktes Duplikat des letzten Ereignisses
            if (cleanFinal === lastFinalSentence) {
              isDuplicate = true;
            }
            // 2. Das Transkript endet bereits mit diesem Satz
            else if (currentTranscript.endsWith(cleanFinal)) {
              isDuplicate = true;
            }

            if (!isDuplicate && cleanFinal.length > 0) {
              transcript.value += (transcript.value ? ' ' : '') + finalChunk;
              lastFinalSentence = cleanFinal; // Speichern für nächsten Check
            }
        ...
        recognition.onerror = (event: SpeechRecognitionErrorEvent) => {
          ...
          // Andere Fehler (no-speech, network) ignorieren wir, damit onend neu startet
        };
    ...
    const toggleRecording = async () => {
      ...
      if (isRecording.value) {
        ...
        recognition.start();
        // Hier nicht transcript resetten, damit man pausieren und weiterreden kann?
        // User-Wunsch: "Neu starten" Button ist für Reset.
        // Also bei Toggle nur weitermachen.
        // Wenn es leer sein soll, muss man 'clearAll' nutzen oder transcript = '' hier setzen.
        // Standard-Verhalten Granola: Neue Session -> oft neuer Text.
        // Ich lasse es hier so, dass es weitermacht (append), außer man drückt refresh.
        // Wenn man neu starten will, nutzt man den Refresh Button.

        // transcript.value = ''; // Auskommentiert, damit man Pause machen kann
        summary.value = '';
        interimTranscript.value = '';
      }
    };
    ...
    const clearAll = () => {
      ...
      summary.value = '';
      isRecording.value = false;
      lastFinalSentence = ''; // Reset Anti-Duplikat
